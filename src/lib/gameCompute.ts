...
export type AchievementTier = 'commun' | 'rare' | 'epique' | 'legendaire'
...
export function computeGame(data: CharacterData): GameCompute {
  ...
  const quests: Quest[] = [
    {
      name: streak.activeToday ? `Série en cours : 🔥 ${streak.current} jour${streak.current > 1 ? 's' : ''}` : `Relance ta série aujourd'hui (dernier record : ${streak.longest} j)`,
      done: streak.activeToday,
      xp: 'action du jour',
    },
