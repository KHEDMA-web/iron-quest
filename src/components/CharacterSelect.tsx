...
export function CharacterSelect({ saveErr, onCreate }: CharacterSelectProps) {
  ...
  const submit = async () => {
    ...
    setChecking(true)
    const available = await isPseudoAvailable(obPseudo.trim())
    setChecking(false)
    if (!available) return setObErr('Ce pseudo est déjà pris par un autre joueur.')
  ...
  return (
    <div className="mx-auto min-h-screen max-w-[520px] px-3.5 pb-10 pt-4">
      <div className="pt-6 text-center">
        ...
        <h1 className="m-0 mt-0.5 font-display text-[38px] font-bold leading-tight">CRÉE TON PERSONNAGE</h1>
      </div>
      ...
      <div className="mt-3.5">
        ...
        {cls && (
          <>
            ...
            <div className="mb-2 flex gap-2">
              {(['H', 'F'] as const).map((v) => (
                <button
                  ...
                  className={`flex-1 rounded-[10px] border px-4 py-2.5 text-sm ${obSex === v ? 'border-accent text-ink' : 'border-line text-muted'}`}
                >
                  {v === 'H' ? 'Homme' : 'Femme'}
                </button>
              ))}
            ...
            >
              {checking ? 'Un instant…' : `LANCER LA PARTIE ${cls.emoji}`}
            </button>
